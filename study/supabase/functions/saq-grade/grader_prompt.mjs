  ...
  properties: {
    parts: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['earned', 'why', 'fix', 'example', 'teacher', 'tea'],
        properties: {
          earned: { type: 'boolean' },
          why: { type: 'string' },
          fix: { type: 'string' },
...
export function systemPrompt() {
  return [
    ...
    '',
    'Everything above is the College Board standard, and the point is decided by it and by nothing else. The student also has a teacher who asks for more on every part; her rules are reported in their own line as advice and never change whether a part earned. Her rules, from her own deck: every part needs a claim, a specific relevant term or event, and an explanation of how they go together, that is all three of T, E and A even on a describe part; label each part; write in complete sentences; do not write in bullet points or fragments; do not quote the excerpt, because the job is analysis and not summary.',
    '',
